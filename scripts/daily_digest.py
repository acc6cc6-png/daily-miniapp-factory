...
def fetch_newsnow_sources(
    ...
    headers = {
        ...
        "User-Agent": "daily-miniapp-factory/1.0",
    }
    try:
...
5. economistTake：120-220字，要像经济分析师，用因果关系解释市场会怎么定价。
    8. investmentSuggestions: 在经济分析师点评后，给出投资建议，包括适合买入的板块/股票，风险提示，并结合当前的经济周期、政策环境、市场情绪综合分析。采用「投资联想法」：分析事件对产业链的传导效应（例：房地产→玻璃期货，棉花→市种率），考虑供需弹性、库存周期、价格传导机制。每个建议必须包含：板块/个股名称、逻辑链条、风险级别、时间周期。最后添加免责声明：“以上建议仅供参考，不构成任何投资建议，投资有风险，入市需谨慎。”
    9. topStories: 编出 8-10 条，每条都必须直接标题即可，title 必须与原始标题完全一致
        10. personaComments: 编出 25 条评论，每条对应一个不同角色，风格必须分明显、要像真实评论区。。7. personaComments：输出 25 条评论，每条对应一个不同角色，风格必须区分明显，要像真实评论区。
